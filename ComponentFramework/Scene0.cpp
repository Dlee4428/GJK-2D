...
Scene0::Scene0(class Window& windowRef):  Scene(windowRef), model0(nullptr) { 
	...
	models = std::vector<Model*>(); //initialize empty vector
}
...
bool Scene0::OnCreate() {
	...
	models.push_back(new Primitive("triangle2.obj"));
	models.push_back(new Primitive("triangle.obj"));
	return true;
}
...
void Scene0::OnDestroy(){
	...
	if (models.size() > 0) {
		...
		models.clear();
		models.shrink_to_fit(); //Make sures models vector shrunk down
	}
}
...
void Scene0::Update(const float deltaTime){
	...
	for (int i = 0; i < models.size(); i++) {
		for (int j = i + 1 ; j < models.size(); j++) {
			Simplex::HandleCollision(*models[i]->GetBody(), *models[j]->GetBody());
		}
	}
	for (int i = 0; i < models.size(); i++) {
